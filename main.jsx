...
const Scriptread = () => {
    ...
    useEffect(() => {
        ...
        const isPaid = params.get('status') === 'success' || localStorage.getItem('sr_paid_final_v5') === 'true';
        if (isPaid) {
            setIsUnlocked(true); setShowPaywall(false); setTotalSeconds(-99999);
            localStorage.setItem('sr_paid_final_v5', 'true');
            if (params.get('status')) window.history.replaceState({}, document.title, window.location.pathname);
        }
        const s_seg = sessionStorage.getItem('sr_seg_v5');
        const s_char = sessionStorage.getItem('sr_char_v5');
        const s_map = sessionStorage.getItem('sr_map_v5');
    ...
    useEffect(() => {
        if (segments.length > 0) {
            sessionStorage.setItem('sr_seg_v5', JSON.stringify(segments));
            sessionStorage.setItem('sr_char_v5', JSON.stringify(characters));
            sessionStorage.setItem('sr_map_v5', JSON.stringify(voiceMap));
        }
    ...
    const handleFirstInteraction = async () => {
        ...
        if (audioContext.current.state === 'suspended') await audioContext.current.resume();
        const msg = "Welcome to Script reed Pro. High fidelity casting scan enabled.";
        try {
    ...
    const guessGenderFromName = (name) => {
        ...
        if (n.endsWith('a') || n.endsWith('ie') || n.endsWith('y') || n.endsWith('ah')) return 'female';
        if (n.endsWith('o') || n.endsWith('r') || n.endsWith('n') || n.endsWith('s')) return 'male';
        return null;
    };
    ...
    const parseScript = async (lines) => {
            ...
            if (isUpper && x > 180 && x < 330 && t.length < 25 && !/ACT|EPISODE|END|TITLE|EXT\.|INT\./i.test(t)) {
                ...
                if (name) {
                    if (!charEvidence.has(name)) {
                        const intro = lines.slice(i, i+15).map(l => l.text).join(" ");
                        charEvidence.set(name, intro);
                    }
        ...
        const aiResults = await analyzeGenders(evidenceArray);

        let newMap = { Narrator: "Serena" };
        charEvidence.forEach((_, name) => {
            let gender = null;
            if (aiResults && aiResults[name]) gender = aiResults[name].toLowerCase();
            if (!gender) gender = guessGenderFromName(name);
            if (!gender) {
                const ev = (charEvidence.get(name) || "").toLowerCase();
                if (/\b(he|him|his|man|father|boy|mr|king|guy)\b/.test(ev)) gender = 'male';
                else if (/\b(she|her|hers|woman|girl|mother|wife|ms|mrs|queen|lady)\b/.test(ev)) gender = 'female';
            }
                ...
                <aside className="w-80 bg-white border-r-2 border-gray-100 flex flex-col shrink-0 overflow-hidden">
                    ...
                    <div className="flex-1 overflow-y-auto p-5 space-y-4 scrollbar-thin">
                        <div className="p-4 bg-gray-50 rounded-xl border">
                            <div className="flex justify-between items-center mb-2"><p className="text-[10px] font-black uppercase text-blue-600">Narrator</p></div>
