...
const Scriptread = () => {
    ...
    const analyzeGenders = async (charList, scriptSnippet) => {
        ...
        try {
            ...
            const prompt = `Analyze this screenplay snippet and the list of characters. For each character, identify if they are 'male' or 'female' based on names and dialogue context (e.g. use of he/she/mr/ms/mother/father). Return ONLY a valid JSON object where keys are character names and values are 'male' or 'female'.\n\nCharacters: ${charList.join(", ")}\n\nScript:\n${scriptSnippet}`;
    ...
    const parseScript = async (lines) => {
        ...
        const scriptSnippet = lines.slice(0, 800).map(l => l.text).join(" ");
        ...
        charArray.forEach(char => {
            ...
            if (genderData && genderData[char]) {
                ...
                // Fallback smart naming
                const n = char.toLowerCase();
                if (n.endsWith('a') || n.endsWith('ee') || n.endsWith('y') || n.includes('dee') || n.includes('danica')) gender = 'female';
                else if (n.endsWith('o') || n.endsWith('r') || n.endsWith('k') || n.includes('robert') || n.includes('zack')) gender = 'male';
                else gender = Math.random() > 0.5 ? 'male' : 'female';
            }
            
            const pool = INWORLD_VOICES[gender];
            const nameHash = char.split('').reduce((acc, c) => acc + c.charCodeAt(0), 0);
            newVoiceMap[char] = pool[nameHash % pool.length].id;
        });
                    ...
                    <div className="bg-white border-2 border-black p-12 shadow-[20px_20px_0px_0px_rgba(37,99,235,1)] max-w-xl rounded-3xl">
                        ...
                        <div className="flex flex-col items-center py-4">
                            <form action="https://www.paypal.com/ncp/payment/QVTMH7RF7NUBE" method="post" target="_blank" style={{display:'inline-grid', justifyItems:'center', alignContent:'start', gap:'0.5rem'}}>
                                <input style={{textAlign:'center',border:'none',borderRadius:'0.25rem',minWidth:'11.625rem',padding:'0 2rem',height:'2.625rem',fontWeight:'bold',backgroundColor:'#FFD140',color:'#000000',cursor:'pointer'}} type="submit" value="Unlock Script - $3.00" />
